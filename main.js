/* ═══════════════════════════════════════════════
   ELITE PROPERTIES KY — Main JavaScript
   Rentec API + Filters + Contact Form
   
   Changes applied per finalization doc:
   - Emails: elitepropertyllcky@gmail.com + Johnlyons1035@gmail.com
   - Phone: 859-325-8778
   - Caption color: #B7C9AF
   - Breathing animation on CTA button
   - Consistent alignment
   - Matterport: jr7xb9xrBE8 + mweATdD8G1L
═══════════════════════════════════════════════ */
...
function buildLargeCard(p) {
  ...
  <div class="prop-card-lg">
    ...
    <div class="prop-card-lg-body">
      ...
      ${desc?`<p class="prop-desc">${desc.substring(0,200)}${desc.length>200?'…':''}</p>`:''}
      ...
      <div style="margin-top:.85rem;padding-top:.85rem;border-top:1px solid var(--cream-dark);">
        <a href="contact.html" style="font-size:.82rem;color:var(--muted);font-weight:500;text-decoration:none;">
  ...
  if (!props.length) {
    grid.innerHTML = `
      <div class="state-center" style="grid-column:1/-1">
        ...
        <a href="${RENTEC_LISTINGS}" target="_blank" class="btn btn-burgundy" style="display:inline-flex">View All on Rentec ↗</a>
      </div>`;
...
function resetRentalsFilter() {
  ['fSearch','fCommunity','fBeds','fBaths','fPrice','fAvail'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
...
function getDemoData() {
  return [
    { name:'Curdsville Rd', address:'Curdsville Rd', city:'Harrodsburg', state:'KY',
      ...
      description:'Newly constructed 3 bedroom, 2 bathroom duplex. Modern finishes, spacious living areas, scenic views in a convenient location.',
    ...
    { name:'Pinehurst Reserve — Unit A', address:'130 Pinehurst Dr', city:'Harrodsburg', state:'KY',
      ...
      description:'Spacious 2 bed 2 bath townhome. Open floor plan with natural light throughout.',
    ...
    { name:'Crozer Townhome', address:'Crozer Lane', city:'Harrodsburg', state:'KY',
      ...
      description:'Contemporary townhome with modern finishes in a convenient Central Kentucky location.',
...
async function loadProperties(opts={}) {
  ...
  try {
    ...
    // 8 second timeout — if proxy is slow, fall back to demo
    ...
    // Auto-populate community dropdown
    if (!communityFilter) {
    ...
    console.log(`✅ Loaded ${props.length} live properties from Rentec`);

  } catch(err) {
    console.warn('Rentec load error:', err.message, '— using demo data');
    ...
    // Soft notice
    const existing = grid.parentElement?.querySelector('.rentec-notice');
    if (!existing) {
      grid.insertAdjacentHTML('afterend', `
        <p class="rentec-notice" style="text-align:center;margin-top:1.25rem;font-size:.83rem;color:var(--muted);">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ['fCommunity','fBeds','fBaths','fPrice','fAvail'].forEach(id => {
    document.getElementById(id)?.addEventListener('change', () => {
      if (document.getElementById('rentalsGrid')) applyRentalsFilter();
      else applyFilters();
    });
  });
